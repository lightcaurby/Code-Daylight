...
run <- function( data.plot, ... )
{
	...
	p <- ggplot( data=data.plot$replacements ) +
		...
		xlab("Location") +
		geom_point( data = data.plot$replacements %>% filter(Vaihdettu), aes( x=Tila, y = VuosiEro, group=Tyyppi, color = Tyyppi ), size=3, alpha=0.5 ) +
		geom_point( data = data.plot$replacements %>% filter(!Vaihdettu), aes( x=Tila, y = VuosiEro, group=Tyyppi, color = Tyyppi ), shape="circle open", size=5, alpha=0.5 ) +
